#!/usr/bin/env bash
# get-token.sh — 从凭证托管服务获取美团旅行助手 access_token
#
# 用法:
#   token=$(bash get-token.sh)
#
# 获取到的 token 会自动写入 ~/.config/meituan-travel/config.json
...
PLATFORM="${CREDENTIAL_PLATFORM:-meituan}"
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "ERROR: HTTP ${HTTP_STATUS}，请先在集成面板中完成美团旅行助手授权" >&2
  exit 1
fi
...
if [[ "$ret" != "0" ]]; then
  echo "ERROR: ret=${ret}，请先在集成面板中完成美团旅行助手授权" >&2
  exit 1
fi
...
if [[ -z "$access_token" || "$access_token" == "null" ]]; then
  echo "ERROR: 未获取到 access_token，请先在集成面板中完成美团旅行助手授权" >&2
  exit 1
fi
...
if [[ "$(uname -s)" == MINGW* || "$(uname -s)" == MSYS* || "$(uname -s)" == CYGWIN* ]]; then
  CONFIG_DIR="${USERPROFILE}/.config/meituan-travel"
else
  CONFIG_DIR="${HOME}/.config/meituan-travel"
fi
