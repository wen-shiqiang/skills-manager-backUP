#!/usr/bin/env python3
"""
合并3个Agent的调研结果，生成调研质量摘要表格。
扫描目标目录下 .research/ 的 01-03 md文件，统计每个维度的来源数量、
一手/二手占比、关键发现。

用法:
    python3 merge_research.py <Agent workspace目录路径>
...
AGENTS = {
    '01-thought-core': '思想内核',
    '02-expression-profile': '表达与互动',
    '03-context-and-perception': '外部视角与来路',
}
...
def main():
    if len(sys.argv) < 2:
        print("用法: python3 merge_research.py <Agent workspace目录路径>")
        sys.exit(1)
    ...
    if not research_dir.exists():
        print(f"❌ 调研目录不存在: {skill_dir / '.research/'}")
        sys.exit(1)
