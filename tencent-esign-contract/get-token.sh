#!/usr/bin/env bash
# get-token.sh — 从凭证托管服务获取腾讯电子签 access_token
#
# 用法:
#   token=$(bash get-token.sh)
#   export ESIGN_TOKEN="$token"
#
...
PLATFORM="${CREDENTIAL_PLATFORM:-tencent_esign}"
...
if [[ "$HTTP_STATUS" != "200" ]]; then
  echo "ERROR: HTTP ${HTTP_STATUS}，请先在集成面板中完成腾讯电子签授权" >&2
  exit 1
fi
...
if [[ "$ret" != "0" ]]; then
  echo "ERROR: ret=${ret}，请先在集成面板中完成腾讯电子签授权" >&2
  exit 1
fi
...
if [[ -z "$access_token" || "$access_token" == "null" ]]; then
  echo "ERROR: 未获取到 access_token，请先在集成面板中完成腾讯电子签授权" >&2
  exit 1
fi
