# get-token.ps1 — Fetch Tencent E-Sign access_token from credential gateway
#
# Usage:
#   $env:ESIGN_TOKEN = & .\get-token.ps1
#
...
$Platform  = if ($env:CREDENTIAL_PLATFORM) { $env:CREDENTIAL_PLATFORM } else { "tencent_esign" }
...
try {
    ...
    [Console]::Error.WriteLine("ERROR: Gateway request failed: $_. Please authorize Tencent E-Sign in the integration panel first.")
    exit 1
}

if ($response.ret -ne 0) {
    [Console]::Error.WriteLine("ERROR: Gateway returned error (ret=$($response.ret)). Please authorize Tencent E-Sign in the integration panel first.")
    exit 1
}
...
if (-not $accessToken -or $accessToken -eq "null") {
    [Console]::Error.WriteLine("ERROR: access_token not found. Please authorize Tencent E-Sign in the integration panel first.")
    exit 1
}
