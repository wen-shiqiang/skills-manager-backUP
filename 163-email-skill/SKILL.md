---
name: 163-email-skill
description: "163 / 126 / 188 / yeah 等网易系邮箱的个人邮箱包装 skill：负责加载本 skill 自有配置，并复用 imap-smtp-email 作为底层引擎"
---

# 📮 163 Email Skill

`163-email-skill` 是 `email-skill` 的下游个人邮箱子 skill，用于承接以下邮箱域名：
...
它**不重复实现 IMAP/SMTP 协议逻辑**，而是：

1. 维护自己独立的 `.env`
2. 根据 163/网易系邮箱预设写入默认服务器配置
3. 通过 `scripts/engine_runner.js` 调用共享底层 `imap-smtp-email`

---

## 1. 什么时候使用

- 需要使用 163 / 126 / 188 / yeah 等网易系邮箱发信
- 需要收件、检索、查看详情、下载附件
- 用户明确指定使用 163 邮箱体系
