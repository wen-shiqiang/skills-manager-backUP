#!/usr/bin/env bash
# get-token.sh — 从凭证网关获取 163 邮箱授权码并写入 .env
set -euo pipefail
...
infer_servers() {
  ...
  case "$domain" in
    ...
    vip.126.com)   IMAP_HOST="imap.vip.126.com";    SMTP_HOST="smtp.vip.126.com" ;;
    188.com)       IMAP_HOST="imap.188.com";         SMTP_HOST="smtp.188.com" ;;
    vip.188.com)   IMAP_HOST="imap.vip.188.com";    SMTP_HOST="smtp.vip.188.com" ;;
    yeah.net)      IMAP_HOST="imap.yeah.net";        SMTP_HOST="smtp.yeah.net" ;;
    qq.com|foxmail.com|vip.qq.com)
      echo "错误: 域名 @${domain} 不属于网易邮箱，请使用 qq-email-skill/get-token.sh" >&2
      exit 1 ;;
    *)
